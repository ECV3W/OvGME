...
GME_Http_Head_Struct GME_NetwHttpParseHead(const char* recv_buff, size_t recv_size)
{
  ...
  for(auto& str : head_entry) // we force all strings to lower case
...
// Callback for download body web page
static size_t write_body(void *ptr, size_t size, size_t nmemb, std::string* string)
{
  
  size_t newLength = size * nmemb;
  string->append((char*)ptr, newLength);
  return newLength;
...
int GME_NetwHttpGET(const char* url_str, const GME_NetwGETOnErr on_err, const GME_NetwGETOnDnl on_dnl, const GME_NetwGETOnSav on_sav, const std::wstring& path)
{
  ...
  /* parse HTTP response header */
  GME_Http_Head_Struct header = GME_NetwHttpParseHead(recv_buff, (size_t)recv_size);
  ...
  recv_size -= (header.size+4);
  if(recv_size) {
    if(fwrite(&recv_buff[header.size+4], 1, recv_size, fp) != (size_t)recv_size) {
      closesocket(sock); WSACleanup();
  ...
  if(header.content_length > body_size) {
    ...
    do {
     ...
     if(fwrite(&recv_buff, 1, recv_size, fp) != (size_t)recv_size) {
        closesocket(sock); WSACleanup();
        fclose(fp); GME_FileDelete(file_path);
        if(on_err) on_err(url_str);
        GME_Logs(GME_LOG_ERROR, "GME_NetwHttpGET", "Body download write error", GME_StrToMbs(file_path).c_str());
...
// For download mod into repository
curlRes GME_NetwHttpGETCurl(const char* url_str, const GME_NetwGETOnErr on_err, const GME_NetwGETOnDnlCurl on_dnl, const GME_NetwGETOnSav on_sav, const std::wstring& path)
{
  CURL *curl_handle;
  ...
  if(!fp) {
    //call error callback?-------------------
    if(on_err) on_err(url_str);
    res.res = CURLE_WRITE_ERROR;
    return res;  // Solid : yeah, that's bad. We'll probably never use REST, so we abuse this error code. Blame me. Nagatsuki here : Solid, here is the right error code instead of CURLE_FTP_COULDNT_USE_REST (I don't blame you though)
  }
  ...
  curl_easy_setopt(curl_handle, CURLOPT_WRITEFUNCTION, write_data);
    ...
    /* get it! */
    res.res = curl_easy_perform(curl_handle);
    ...
    if(res.res == CURLE_OK )
    {
      /*Check if http code error is good or not*/
...
// For getting the list with all link into the repository
curlRes GME_NetwHttpGETCurl(const char* url_str, const GME_NetwGETOnErr on_err,  const GME_NetwGETOnDnlCurl on_dnl, const GME_NetwGETOnEnd on_end)
{
  ...
  else
  {
    ...
    if (res.http_code == 200)
      /* send data to callback */
      if(on_end) on_end(body_data.c_str(), body_data.size());
  }
