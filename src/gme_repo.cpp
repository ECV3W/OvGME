...
void GME_RepoDnl_OnErr(const char* url)
{
  ...
  SetDlgItemTextA(g_hwndRepUpd, TXT_DOWNSPEED, "---- MByte/s");
...
bool GME_RepoDnl_OnDnl(unsigned percent, unsigned rate)
{
  ...
  SetDlgItemTextA(g_hwndRepUpd, TXT_DOWNSPEED, buff);
                    ...
                    curl_off_t ultotal, curl_off_t ulnow)
{
  ...
  /* Use a local high-resolution timer (GetTickCount64 in milliseconds)
     to throttle UI updates Update every 30 ms to keep the
     progress bar responsive. */
  curtime = (curl_off_t)GetTickCount64();
  const curl_off_t UI_UPDATE_MS = 30; /* 30 ms */
  if((curtime - progressData->lastruntime) >= UI_UPDATE_MS) {
    progressData->lastruntime = curtime;

    /* calculate percentage*/
    /* Make percentage calculation robust:
       - dltotal may be 0 or unknown for some transfers (chunked/unknown length).
       - Try to get a better total from CURLINFO_CONTENT_LENGTH_DOWNLOAD_T when needed.
       - Clamp percent to the 0..100 range to avoid premature non-100 finish. */
    curl_off_t total = dltotal;
    if(total <= 0) {
      /* try to recover content-length reported by libcurl */
      curl_easy_getinfo(curl, CURLINFO_CONTENT_LENGTH_DOWNLOAD_T, &total);
    }
    if(total > 0) {
      percent = (float)dlnow * 100.0f / (float)total;
      if(percent < 0.0f) percent = 0.0f;
      if(percent > 100.0f) percent = 100.0f;
    } else {
      /* unknown total: set percent to 0.. keep UI from showing incorrect values */
      percent = 0.0f;
    }
  ...
  /* update  views */
  HWND hpb = GetDlgItem(g_hwndRepUpd, PBM_DONWLOAD);
  /* round percent to integer 0..100 */
  /* avoid dependency on math lib functions here: emulate ceilf for positive percent */
  int ipercent = (int)(percent + 0.999f);
  if(ipercent < 0) ipercent = 0;
  if(ipercent > 100) ipercent = 100;
  /* Use SendMessage to update the progress position and force an immediate repaint
     so the control doesn't animate slowly and lag behind fast downloads. */
  SendMessage(hpb, PBM_SETPOS, (WPARAM)ipercent, 0);
  InvalidateRect(hpb, NULL, TRUE);
  UpdateWindow(hpb);
  GME_RepoDnl_SetItemProgress(g_GME_ReposDnl_List[g_ReposQry_Id].name, ipercent);

    /* get download speed */
    curl_easy_getinfo(curl, CURLINFO_SPEED_DOWNLOAD_T , &avgDnlRateBs);
    if ((float)(avgDnlRateBs / 1024 / 1024) > 1.0f)
    {
      dnlRate = (float)avgDnlRateBs / 1024 / 1024;
      sprintf(buff, "%.1f MByte/s",dnlRate);
    }
    else if ((float)(avgDnlRateBs / 1024) > 1.0f)
    {
      dnlRate = (float)avgDnlRateBs / 1024;
      sprintf(buff, "%.1f KByte/s",dnlRate);
    }
    else
    {
      dnlRate = (float)avgDnlRateBs;
      sprintf(buff, "%d Byte/s", (int)dnlRate);
    }
  ...
  for(unsigned i = 0; i < g_GME_ReposDnl_List.size(); i++)
  {
    ...
    if(curl_result.res)
    {
      ...
      case CURLE_ABORTED_BY_CALLBACK:
        ...
        break;
      default:
        const char* curl_err_msg = curl_easy_strerror(curl_result.res);
    ...
    if (curl_result.http_code != 200 && curl_result.http_code != 0) // Exclude 0 for avoid double error if curl had already an error
    {
...
DWORD WINAPI GME_RepoQueryUpd_Th(void* args)
{
  ...
  for(unsigned i = 0; i < g_GME_Repos_List.size(); i++) {
    ...
    SetDlgItemTextA(g_hwndRepUpd, TXT_REPOQRYURL, cnx_msg.c_str());

    GME_Logs(GME_LOG_NOTICE, "GME_RepoQueryUpd_Th", "Try HTTP access", xml_url.c_str());

    curl_result = GME_NetwHttpGETCurl(xml_url.c_str(), GME_RepoUpd_OnErr, GME_RepoDnl_OnDnlCurl, GME_RepoUpd_OnEnd);
    if(curl_result.res)
    {
      /* error received from curl */
      bool showdialog = true;
...
std::string GME_RepoMakeXml(const char* url_str, bool cust_path, const wchar_t* path_str)
{
  ...
  SetDlgItemTextA(g_hwndRepXml, ENT_OUTPUT, xml_ascii.c_str());
...
bool GME_RepoSaveXml()
{
  ...
  if(fp) {
    ...
    GetDlgItemTextA(g_hwndRepXml, ENT_OUTPUT, (char*)xml_src, xml_src_size+1);
...
bool GME_RepoTestXml(const wchar_t* path, unsigned offst)
{
  ...
  if(GME_FileGetAsciiContent(file_path, &content) > 0) {
    ...
    if(!GME_RepoParseXml(content, &reposmod_list, &log)) {
      SetDlgItemTextA(g_hwndRepXts, TXT_MESSAGE, "XML repository file parsing error occurred, the repository XML source file is not valid.");
      SetDlgItemTextA(g_hwndRepXts, ENT_OUTPUT, log.c_str());
      return false;
    }

    SetDlgItemTextA(g_hwndRepXts, TXT_MESSAGE, "XML repository file parsing succeed, the repository XML source file appear valid.");
    ...
    SetDlgItemTextA(g_hwndRepXts, ENT_OUTPUT, output.c_str());
