...
{
  while(true) {
    if(WaitForSingleObject(g_ModsDir_hChge, INFINITE) == WAIT_OBJECT_0 && !g_ReposQry_Running) {
      GME_ModsUpdList();
...
bool GME_GameNewCfg(const std::wstring& title, const std::wstring& root, const std::wstring& mods, bool use_custom_back, const std::wstring& backp)
{
  ...
  if(use_custom_back) {
    ...
    for(long long unsigned int i = 0; i < g_GameCfg_List.size(); i++) {
      if(i != (long long unsigned int)g_GameCur_Id) {
    ...
    do {
      if(fdw.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
        ...
        if(cfg_fp) {
          ...
          // Because of the mooving of the home directory into saved games folder. I need to detect and change all conf file Game.dat with the new path in it for people who moove their folder too
          pos_path_dir = cfg_path.find(L"\\OvGME\\"); // we firstly parse cfg_path for get the root folder
          cfg_path_dir = cfg_path.substr(0, pos_path_dir);
          std::wstring conf_dir(cfg_data.conf_dir); // Then we parse the path into game.dat
          std::wstring back_dir(cfg_data.back_dir);
          pos_root_conf_dir = conf_dir.find(L"\\OvGME\\");
          root_conf_dir = conf_dir.substr(0, pos_root_conf_dir);
          name_conf_dir = conf_dir.substr(pos_root_conf_dir + 7);
          name_back_dir = back_dir.substr(pos_root_conf_dir + 7);
          if (cfg_path_dir != root_conf_dir) // Then we compare the 2 root folder
          {
            //Edit the right path
            swprintf(cfg_data.conf_dir, 260, L"%ls\\OvGME\\%ls", cfg_path_dir.c_str(), name_conf_dir.c_str());
            swprintf(cfg_data.back_dir, 260, L"%ls\\OvGME\\%ls", cfg_path_dir.c_str(), name_back_dir.c_str());
            //Place the cursor at the right place for conf_dir and back_dir then rewrite the data with the good path
            offset = offsetof(GME_GameCfg_Struct, conf_dir);
            fseek(cfg_fp, offset, SEEK_SET);
            fwrite(cfg_data.conf_dir, sizeof(wchar_t), 260, cfg_fp);
            offset = offsetof(GME_GameCfg_Struct, back_dir);
            fseek(cfg_fp, offset, SEEK_SET);
            fwrite(cfg_data.back_dir, sizeof(wchar_t), 260, cfg_fp);
