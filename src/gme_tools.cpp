...
/*
  function to get path to OvGME home (Save Game) dir
*/
std::wstring GME_GetAppdataPath()
{
  PWSTR buff = nullptr;
  //SHGetFolderPathW(NULL, CSIDL_LOCAL_APPDATA, NULL, 0, buff);
  SHGetKnownFolderPath(FOLDERID_SavedGames, 0, NULL, &buff);
...
/*
  Function for load an Image like png image using WIC for load from .rc file
*/

HRESULT GME_LoadImage(HINSTANCE const&hInstance,
    PCWSTR resourceName,
    PCWSTR resourceType,
    IWICImagingFactory* pFactory,
    IWICBitmapSource** ppBitmapSource)
{
  if (!pFactory || !ppBitmapSource) return E_POINTER;
  *ppBitmapSource = nullptr;
  ...
  if (!hData) return HRESULT_FROM_WIN32(GetLastError());
  
  void* pData = LockResource(hData);
  if (!pData) return E_FAIL;
  
  // Create a memory stream on ressource's data
  Microsoft::WRL::ComPtr<IStream> spStream;
  HRESULT hr = CreateStreamOnHGlobal(NULL, TRUE, &spStream);
  if (FAILED(hr)) return hr;
  
  ULONG written = 0;
  hr = spStream->Write(pData, resSize, &written);
  if (FAILED(hr) || written != resSize) return E_FAIL;
  
  LARGE_INTEGER zero = {};
  hr = spStream->Seek(zero, STREAM_SEEK_SET, nullptr);
  if (FAILED(hr)) return hr;
  
  // Create decoder from stream
  Microsoft::WRL::ComPtr<IWICBitmapDecoder> spDecoder;
  hr = pFactory->CreateDecoderFromStream(spStream.Get(), nullptr, WICDecodeMetadataCacheOnLoad, &spDecoder);
  if (FAILED(hr)) return hr;
  
  // Extract the first frame
  Microsoft::WRL::ComPtr<IWICBitmapFrameDecode> spFrame;
  hr = spDecoder->GetFrame(0, &spFrame);
  if (FAILED(hr)) return hr;
  
  // Return the frame as source bitmap
  *ppBitmapSource = spFrame.Detach();
...
/*
  ...
  Function for add the loaded image into the gui

*/
void GME_AddImage(HINSTANCE const&hInstance, HBITMAP hbitmap, HWND const&parentdlg, int const&idCtrlStatic)
{
  ...
  HRESULT hr = GME_LoadImage(
      hInstance,
      MAKEINTRESOURCEW(idCtrlStatic),  // nom/id of the ressource PNG in .rc
      L"PNG",                      // PNG type indicate into the .rc
      pFactory,
      &pBitmapSource);
  
  // If successeful, we convert our image into hbitmap and call a SendMessage
  if (SUCCEEDED(hr) && pBitmapSource)
  {
    
    HWND hstatictrdmain = GetDlgItem(parentdlg, idCtrlStatic);
    
    hbitmap = GME_WICBitmapToHBitMap(pBitmapSource);

    SendMessage(hstatictrdmain, STM_SETIMAGE, IMAGE_BITMAP, (LPARAM)hbitmap);

  }

  // Free object COM after use
  if (pBitmapSource) pBitmapSource->Release();
  if (pFactory) pFactory->Release();

}

HBITMAP GME_WICBitmapToHBitMap(IWICBitmapSource* pBitmapSource)
{

  if (!pBitmapSource) return NULL;

  UINT width = 0;
  UINT height = 0;
  if (FAILED(pBitmapSource->GetSize(&width, &height)) || width == 0 || height == 0) {
      return NULL;
  }

  // Prepare BITMAPINFO for the DIBSection 32 bits (BGRA format)
  BITMAPINFO bminfo = {};
  bminfo.bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
  bminfo.bmiHeader.biWidth = static_cast<LONG>(width);
  bminfo.bmiHeader.biHeight = -static_cast<LONG>(height);  // Negative height = top-down
  ...
  HDC hdc = GetDC(NULL);
  HBITMAP hBitmap = CreateDIBSection(hdc, &bminfo, DIB_RGB_COLORS, &pvPixels, NULL, 0);
  ReleaseDC(NULL, hdc);
  ...
  // Pixels copy from IWICBitmapSource into DIBSection
  const UINT stride = width * 4;  // 4 bytes per pixel (32bpp)
  const UINT bufferSize = stride * height;

  HRESULT hr = pBitmapSource->CopyPixels(NULL, stride, bufferSize, reinterpret_cast<BYTE*>(pvPixels));
  if (FAILED(hr)) {
