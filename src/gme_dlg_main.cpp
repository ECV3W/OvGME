...
void GME_MainInit()
{
  ...
  /* create 3rd image */
  GME_AddImage(g_hInst, g_htrdMain, g_hwndMain, IMG_THIRD);
...
void GME_MainPopMenuMods()
{
  ...
  if(single) {
    AppendMenuW(hMenuPopup, MF_STRING, POP_TOGGLE, L"Toggle");
    AppendMenuW(hMenuPopup, MFT_SEPARATOR, MFT_SEPARATOR, L"---");
    AppendMenuW(hMenuPopup, MF_STRING, POP_OPENDIR, L"Explore...");
    AppendMenuW(hMenuPopup, MF_STRING, POP_MODDELETE, L"Delete...");
    AppendMenuW(hMenuPopup, MFT_SEPARATOR, MFT_SEPARATOR, L"---");
    AppendMenuW(hMenuPopup, MF_STRING, POP_MODCREATE, L"Make Mod-Archive...");
  } else {
    AppendMenuW(hMenuPopup, MF_STRING, POP_TOGGLE, L"Toggle selected");
    AppendMenuW(hMenuPopup, MF_STRING, POP_ENABLE, L"Enable selected");
    AppendMenuW(hMenuPopup, MF_STRING, POP_DISABLE, L"Disable selected");
  }
...
void GME_DlgMainResize()
{
  ...
  SetWindowPos(GetDlgItem(hwd, BOX_MAIN), NULL, 10, 110, cli.right-20, cli.bottom-147, SWP_NOZORDER);
  SetWindowPos(GetDlgItem(hwd, BTN_QUIT), NULL, cli.right-90, cli.bottom-30, 80, 23, SWP_NOZORDER);
  SetWindowPos(GetDlgItem(hwd, ENT_MODDESC), NULL, 20, cli.bottom-273, cli.right-40, 225, SWP_NOZORDER);
  SetWindowPos(GetDlgItem(hwd, LAB_MODDESC), NULL, 20, cli.bottom-288, 150, 15, SWP_NOZORDER);
  SetWindowPos(GetDlgItem(hwd, BTN_MODDIS), NULL, 20, cli.bottom-316, 100, 23, SWP_NOZORDER);
  SetWindowPos(GetDlgItem(hwd, BTN_MODENA), NULL, 120, cli.bottom-316, 100, 23, SWP_NOZORDER);
  SetWindowPos(GetDlgItem(hwd, BTN_MODCANCEL), NULL, cli.right-100, cli.bottom-315, 80, 23, SWP_NOZORDER);
  SetWindowPos(GetDlgItem(hwd, PBM_MODPROC), NULL, 225, cli.bottom-315, cli.right-330, 22, SWP_NOZORDER);

  SetWindowPos(GetDlgItem(hwd, LVM_MODSLIST), NULL, 20, 174, cli.right-40, cli.bottom-500, SWP_NOZORDER);
  ...
  SetWindowPos(GetDlgItem(hwd, CHK_SORTMODS), NULL, 20, 120, cli.right-40, 20, SWP_NOZORDER);

  SetWindowPos(GetDlgItem(hwd, BTN_IMPORTMOD), NULL, cli.right-105, 142, 85, 23, SWP_NOZORDER);
  SetWindowPos(GetDlgItem(hwd, ENT_MODSPATH), NULL, 20, 143, cli.right-130, 21, SWP_NOZORDER);

  SetWindowPos(GetDlgItem(hwd, BOX_TOP), NULL, 268, 15, cli.right-278, 80, SWP_NOZORDER);
  SetWindowPos(GetDlgItem(hwd, CMB_GAMELIST), NULL, 278, 35, cli.right-375, 22, SWP_NOZORDER);
  SetWindowPos(GetDlgItem(hwd, BTN_EDIGAME), NULL, cli.right-92, 34, 35, 23, SWP_NOZORDER);
  SetWindowPos(GetDlgItem(hwd, BTN_ADDGAME), NULL, cli.right-55, 34, 35, 23, SWP_NOZORDER);
  SetWindowPos(GetDlgItem(hwd, ENT_CFGRPATH), NULL, 278, 60, cli.right-298, 21, SWP_NOZORDER);
}
...
BOOL CALLBACK GME_DlgMain(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_INITDIALOG:
    ...
    // Need to initialise COM for WIC and import PNG image
